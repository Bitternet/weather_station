...
void QcLabelItem::draw(QPainter *painter)
{
    ...
    QFont font("Meiryo UI", r/(7.0*mSize), QFont::Bold);
...
void QcLabelItem::setSize(float m)
{
    mSize = m;
    update();
}
...
void QcArcItem::draw(QPainter *painter)
{
    ...
    if(r/50 < 3)
    pen.setWidthF(3);
    else pen.setWidthF(r/50);
    painter->setPen(pen);
    painter->drawArc(tmpRect,-mKof*(mMinDegree+180),-mKof*(mMaxDegree-mMinDegree));
}
...
void QcDegreesItem::draw(QPainter *painter)
{
    ...
    for(float val = mMinValue; val<=mMaxValue; val+=mStep){
        ...
        pt = path.pointAtPercent(pointAtstart);
        QPointF newPt = path.pointAtPercent(pointAtfinish);
...
void QcDegreesItem::setpointAtstart(double pstrart)
{
    pointAtstart = pstrart;
    update();
}

void QcDegreesItem::setpointAtfinish(double pfinish)
{
    pointAtfinish = pfinish;
    update();
}
...
void QcNeedleItem::createDiamonNeedle(float r)
{
    ...
    tmpPoints.append(QPointF(-r/5.0,r));
    tmpPoints.append(QPointF(0.0,r/1.3));
